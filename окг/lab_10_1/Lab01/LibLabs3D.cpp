...
// Index=1 -  Зеркальная модель отражения света :[cos(a)]^n
{
	...
	COLORREF Col;   // Цвет пикселя

	double df = 0.5;                            // Шаг по углу фи
	double dq = 0.5;                            // Шаг по углу тета
	double kLight;
	...
	CMatrix MV = CreateViewCoord(PView(0), PView(1), PView(2));	    // Матрица(4 на 4) пересчета в видовую систему координат
    ...
    for (double fi = 0; fi <= 360.0; fi += df) {
        for (double q = 0; q <= 180.0; q += dq)
        {
            ...
            double cos_RN = CosV1V2(VK, VSphereNorm);   // Косинус угла между вектором точки наблюдения
                                                        // и вектором нормали к точке сферы
            ...
            if (cos_RN > 0)             // Если точка видима наблюдателю...
            {
                ...
                CMatrix VP = VS - VSphereNorm;          // Вектор, определяющий направление света 
                                                        // от точки на сфере до источника света.
                ...
                if (cos_PN > 0)	            // Если точка сферы освещается...
                {
                    ...
                    {
                        // VP - вектор, определяющий направление на источник света
                        // VSphereNorm - вектор нормали к элементу поверхности
                        // RX - вектор, направление отраженного луча
